...
function exportedPaths(pkg) {
	const paths = {};
	if (pkg.exports) {
		for (var key in pkg.exports) {
			if (key == "import") paths['.'] = pkg.exports[key];
			else if (key.startsWith(".")) paths[key] = pkg.exports[key];
		}
	} else {
		const fallback = pkg.module || pkg['jsnext:main'] || pkg.main;
		if (fallback) paths["."] = fallback;
	}
	return paths;
}
