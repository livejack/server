...
async function queryLogin(req, res) {
	...
	if (token) {
		const user = await Models.User.query().findOne({ domain });
		if (await verifyPassword(user, token)) {
...
async function verifyPassword(user, token) {
	if (!user || !user.tokens) return false;
	return user.tokens.some(item => item == token);
}
